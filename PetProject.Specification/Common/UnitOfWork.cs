...
namespace PetProject.Specification.Common
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        public UnitOfWork(IDataContext dataContext)
        {
            ...
            _dbContext = dataContext.Context;
        }
        ...
        public virtual IGenericRepository<TEntity> GetRepository<TEntity>() where TEntity : BaseEntity
        {
            ...
            value = CreateRepository<TEntity>();
            if (value == null)
            {
        ...
        private object? CreateRepository<TEntity>() where TEntity : BaseEntity
        {
            var typeRepository = typeof(IGenericRepository<TEntity>);
            var typeAssignabledRespository = AppDomain.CurrentDomain.GetAssemblies()
                .SelectMany(s => s.GetTypes())
                .FirstOrDefault(t => typeRepository.IsAssignableFrom(t)
                );
            if (typeAssignabledRespository == null)
            {
                return Activator.CreateInstance(typeof(GenericRepository<>).MakeGenericType(typeof(TEntity)), _dbContext);
            }
            else
            {
                return Activator.CreateInstance(typeAssignabledRespository, _dbContext);
            }
        }
